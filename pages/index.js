...
export const getServerSideProps = async () => {
  const nasaRes = await fetch(
    "https://api.nasa.gov/planetary/apod?api_key=4D3DfuIitOHR4LyC1lsQYo70iFPpJ67oDXG4TJw0"
  );
...
export default function Home({ allData: { nasaData, quoteData } }) {
  return (
    ...
    <main
      class="flex justify-center items-center w-screen h-screen bg-no-repeat bg-cover bg-center"
      style={{
        backgroundImage:
          nasaData.url && nasaData.media_type === "image"
            ? `url('${nasaData.url}')`
            : "url(/galaxy.jpg)",
      }}
    >
      ...
      <section class="max-w-md rounded-xl p-10 m-10 bg-gradient-to-t from-black/50 to-black/20 text-white font-serif md:p-16 md:max-w-xl lg:max-w-3xl xl:max-w-5xl">
        ...
        {quoteData[0].a ? (
          <span class="block text-xl mt-5 md:text-2xl md:mt-7 lg:mt-10">
            - {quoteData[0].a}
          </span>
        ) : (
